...
do
    ...
    for _IT in {1..10}
    do
        (cd ../rust/extended_seive && /usr/bin/time -f %U -o ../../script/out.out cargo run --release $INC > /dev/null)
        RTIM=$(< out.out)
    ...
    for _IT in {1..10}
    do
        (cd ../cpp/extended_sieve && /usr/bin/time -f %U -o ../../script/out.out ./sieve $INC > /dev/null)
        CTIM=$(< out.out)
...
do
    ...
    for _IT in {1..10}
    do
        (cd ../rust/pi_approx && /usr/bin/time -f %U -o ../../script/out.out cargo run --release $INC > /dev/null)
        RTIM=$(< out.out)
        SUMR=$(echo "$SUMR + $RTIM" | bc)
    done
    ...
    for _IT in {1..10}
    do
        (cd ../cpp/pi_approx && /usr/bin/time -f %U -o ../../script/out.out ./pi_approx $INC > /dev/null)
        CTIM=$(< out.out)
        SUMC=$(echo "$SUMC + $CTIM" | bc)
    done
